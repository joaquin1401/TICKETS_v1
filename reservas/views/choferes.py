...
def chofer_dashboard(request):
    ...
    tickets_en_curso = (
        Ticket.objects.filter(estado=Ticket.ESTADO_EN_CURSO, conductor=usuario)
...
def aceptar_ticket(request, ticket_id):
    ...
    if (
        Ticket.objects.filter(conductor=usuario, estado=Ticket.ESTADO_EN_CURSO)
    ...
    if ticket.estado == Ticket.ESTADO_APROBADO and ticket.conductor is None:
        ...
        if not km_inicio_str:
            messages.error(
                request,
                "Debes ingresar el kilometraje de inicio para comenzar el viaje.",
            )
        ...
        messages.success(
            request,
            f"Te has asignado como conductor del ticket #{ticket.pk} y comenzaste el viaje.",
        )
    else:
    ...
    if ticket.conductor == usuario or request.session.get("es_admin"):
        if ticket.estado == Ticket.ESTADO_EN_CURSO:
            ...
            if not km_fin_str or not hora_fin_real_str:
                messages.error(
                    request,
                    "Debes ingresar todos los datos reales (km y horarios) para finalizar.",
                )
                return redirect(request.META.get("HTTP_REFERER", "inicio"))
            ...
            from django.utils.timezone import is_aware, is_naive, localtime, make_aware

            try:
                ticket.kilometraje_fin = Decimal(km_fin_str)
                if (
                    ticket.kilometraje_fin >= Decimal("100000000")
                    or ticket.kilometraje_fin < 0
                ):
                    messages.error(
                        request,
                        "El kilometraje ingresado es inválido o demasiado grande (máximo 8 dígitos enteros).",
                    )
                    return redirect(request.META.get("HTTP_REFERER", "inicio"))
            ...
            if ticket.hora_fin_real < ticket.hora_inicio_real:
                messages.error(
                    request,
                    "La hora de regreso no puede ser anterior a la hora de salida.",
                )
                return redirect(request.META.get("HTTP_REFERER", "inicio"))

            if (
                ticket.kilometraje_inicio is not None
                and ticket.kilometraje_fin < ticket.kilometraje_inicio
            ):
                messages.error(
                    request,
                    f"El kilometraje de regreso no puede ser menor al de salida ({ticket.kilometraje_inicio}).",
                )
                return redirect(request.META.get("HTTP_REFERER", "inicio"))
            ...
            if ticket.hora_fin:
                now_local = (
                    ...
                    if is_aware(timezone.now())
                    else timezone.now()
                )
                fin_local = (
                    localtime(ticket.hora_fin)
                    if is_aware(ticket.hora_fin)
                    else ticket.hora_fin
                )
                ...
                if retraso.total_seconds() > 7200:
                    ...
                    if not justificacion:
                        messages.error(
                            request,
                            "El viaje finalizó con más de 2 horas de retraso real. Debe ingresar una justificación obligatoria.",
                        )
                        return redirect(request.META.get("HTTP_REFERER", "inicio"))
