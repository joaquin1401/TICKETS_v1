        ...
        if action == "add_feriado":
            ...
            if fecha_str:
                try:
                    ...
                    if not Feriado.objects.filter(fecha=fecha).exists():
                        ...
                        messages.success(
                            request,
                            f"Feriado del {fecha.strftime('%d/%m/%Y')} agregado correctamente.",
                        )
                    else:
            ...
            if not csv_file:
                ...
                try:
                    ...
                    for index, row in enumerate(reader):
                        ...
                        if index == 0 and (
                            "fecha" in str(row).lower() or "date" in str(row).lower()
                        ):
                        ...
                        if len(row) >= 1:
                            try:
                                ...
                                feriado, created = Feriado.objects.get_or_create(
                                    fecha=fecha_obj, defaults={"descripcion": desc}
                                )
                                if created:
                                    agregados += 1
                                else:
                    ...
                    if agregados > 0 and repetidos == 0:
                        ...
                        if errores > 0:
                            msg += f" ({errores} errores de formato)."
                        ...
                        msg = "No se agregaron fechas nuevas. Las fechas del archivo ya estaban registradas."
                        if errores > 0:
                            msg += f" ({errores} errores de formato)."
                        ...
                        if errores > 0:
                            msg += f" ({errores} filas ignoradas por error de formato)."
                        messages.error(request, msg)
                except Exception as e:
                    messages.error(
                        request, f"Error al procesar el archivo CSV: {str(e)}"
                    )
            return redirect("configuracion_global")
            ...
            try:
                ...
                for dt, name in ar_holidays.items():
                    feriado, created = Feriado.objects.get_or_create(
                        fecha=dt, defaults={"descripcion": name}
                    )
                    if created:
                        agregados += 1
                    else:
                ...
                if agregados > 0 and repetidos == 0:
                    messages.success(
                        request,
                        f"Se sincronizaron los feriados del año {anio} exitosamente. Se agregaron {agregados} feriados nuevos.",
                    )
                elif agregados > 0 and repetidos > 0:
                    messages.warning(
                        request,
                        f"Se agregaron {agregados} feriados nuevos del {anio}, pero {repetidos} ya estaban registrados.",
                    )
                elif agregados == 0 and repetidos > 0:
                    messages.error(
                        request,
                        f"No se agregaron feriados nuevos del {anio}. Todos ya estaban registrados en el sistema.",
                    )
                else:
                    messages.error(
                        request, f"No se encontraron feriados para el año {anio}."
                    )
            except Exception as e:
            ...
            if form.is_valid():
                form.save()
                messages.success(
                    request, "La configuración se actualizó correctamente."
                )
                return redirect("configuracion_global")
    else:
...
def preview_email(request, template_name):
    ...
    try:
        ...
        return HttpResponse(
            f"Error cargando plantilla '{template_name}': {e}", status=404
        )
