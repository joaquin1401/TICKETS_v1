...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING("🔄 Iniciando poblamiento de base de datos...\n")
        )
    ...
    def _crear_usuarios(self, cargos):
        ...
        usuarios_config = [
            ...
            # Decanos
            {
                "nombre": "Dorotea",
                "apellido": "Lucena",
                "correo": "decano_aprobado@universidad.edu",
                "cargo": Cargo.DECANO,
                "valido": True,
                "rechazado": False,
            },
            {
                "nombre": "Ani",
                "apellido": "Laguna",
                "correo": "decano_pendiente@universidad.edu",
                "cargo": Cargo.DECANO,
                "valido": False,
                "rechazado": False,
            },
            {
                "nombre": "Danilo",
                "apellido": "Barba",
                "correo": "decano_rechazado@universidad.edu",
                "cargo": Cargo.DECANO,
                "valido": False,
                "rechazado": True,
            },
            # Secretarios
            {
                "nombre": "Celia",
                "apellido": "Mascaró",
                "correo": "secretario_aprobado@universidad.edu",
            ...
            },
            {
                "nombre": "Jerónimo",
                "apellido": "Pareja",
                "correo": "secretario_pendiente@universidad.edu",
                "cargo": Cargo.SECRETARIO,
                "valido": False,
                "rechazado": False,
            },
            {
                "nombre": "Anastasia",
                "apellido": "Rueda",
                "correo": "secretario_rechazado@universidad.edu",
                "cargo": Cargo.SECRETARIO,
                "valido": False,
                "rechazado": True,
            },
            # Usuarios regulares
            {
                "nombre": "Nydia",
                "apellido": "Pereira",
                "correo": "usuario_aprobado@universidad.edu",
                "cargo": Cargo.USUARIO,
                "valido": True,
                "rechazado": False,
            },
            {
                "nombre": "Jaime",
                "apellido": "Ferrera",
                "correo": "usuario_pendiente@universidad.edu",
                "cargo": Cargo.USUARIO,
                "valido": False,
                "rechazado": False,
            },
            {
                "nombre": "Wilfredo",
                "apellido": "Iglesia",
                "correo": "usuario_rechazado@universidad.edu",
                "cargo": Cargo.USUARIO,
                "valido": False,
                "rechazado": True,
            },
            # Choferes
            {
                "nombre": "Carlos",
                "apellido": "Piloto",
                "correo": "chofer1@universidad.edu",
                "cargo": Cargo.CHOFER,
                "valido": True,
                "rechazado": False,
            },
            {
                "nombre": "Miguel",
                "apellido": "Rueda",
                "correo": "chofer2@universidad.edu",
                "cargo": Cargo.CHOFER,
                "valido": True,
                "rechazado": False,
            },
        ...
        for config in usuarios_config:
            ...
            if cargo_nombre not in cargos:
                self.stdout.write(
                    f"  [Warning] Cargo {cargo_nombre} no encontrado, omitiendo usuario {config['correo']}"
                )
                continue
            ...
            simbolo = (
                "✓" if config["valido"] else ("✗" if config["rechazado"] else "⏳")
            )
            if cargo_nombre == Cargo.ADMIN_SEU:
                simbolo = "⭐"

            self.stdout.write(
                f"  {simbolo} {usuario.nombre_completo} ({cargo_nombre}) - {estado_str} [{usuario.correo}]"
            )
    ...
    def _crear_reservas(self, usuarios, vehiculos):
        ...
        for _i in range(4):
            ticket = self._crear_reserva(
                ...
                vehiculo=random.choice(vehiculos),
                hora_inicio=ahora
                + timedelta(days=random.randint(1, 15), hours=random.randint(8, 18)),
                duracion_horas=3,
                estado=Ticket.ESTADO_APROBADO,
        ...
        if 3 in usuarios_por_prioridad and usuarios_por_prioridad[3]:
            ...
            if ticket_bajo:
                contador_creados += 1
                self.stdout.write(
                    f"    ✓ Ticket #{ticket_bajo.id}: {usuario_bajo.nombre_completo} (Usuario) reserva"
                )
        ...
        if 2 in usuarios_por_prioridad and usuarios_por_prioridad[2]:
            ...
            if resultado.exito:
                contador_creados += 1
                self.stdout.write(
                    f"    ✓ Ticket #{resultado.ticket.id}: Secretario SOBRESCRIBE (mayor prioridad)"
                )
            else:
        ...
        if 1 in usuarios_por_prioridad and usuarios_por_prioridad[1]:
            ...
            if resultado.estado == resultado.SOBRESCRITO:
                ...
                contador_creados += 1
                self.stdout.write(
                    f"    ✓ Ticket #{resultado.ticket.id}: Decano aprobado sin conflictos"
                )
            else:
        ...
        for _i in range(max(0, tickets_faltantes + 5)):  # Asegurar más de 100
            ...
            ticket = self._crear_reserva(
                ...
                hora_inicio=ahora
                + timedelta(
                    days=random.randint(-40, 40), hours=random.randint(-12, 12)
                ),
    ...
    def _crear_reserva(
        ...
        hora_fin = (
            ...
            else hora_inicio + timedelta(hours=duracion_horas)
        )
        ...
        if estado == Ticket.ESTADO_FINALIZADO:
            ...
            kilometraje_fin = round(
                kilometraje_inicio + (distancia_est * random.uniform(0.9, 1.1)), 2
            )
            ...
            if hora_fin:
                ...
                hora_fin_real = hora_inicio_real + timedelta(
                    hours=duracion_horas if duracion_horas else 2
                )
    ...
    def _mostrar_resumen(self):
        ...
        self.stdout.write(
            f"   - Aprobados: {Usuario.objects.filter(valido=True).count()}"
        )
        self.stdout.write(
            f"   - Pendientes: {Usuario.objects.filter(valido=False, rechazado=False).count()}"
        )
        self.stdout.write(
            f"   - Rechazados: {Usuario.objects.filter(rechazado=True).count()}"
        )

        self.stdout.write(f"\n🚐 Vehículos: {Vehiculo.objects.count()}")
        self.stdout.write(
            f"   - Disponibles: {Vehiculo.objects.filter(activo=True).count()}"
        )
        self.stdout.write(
            f"   - En mantenimiento: {Vehiculo.objects.filter(activo=False).count()}"
        )

        self.stdout.write(f"\n🎫 Tickets/Reservas: {Ticket.objects.count()}")
        self.stdout.write(
            f"   - Aprobados: {Ticket.objects.filter(estado=Ticket.ESTADO_APROBADO).count()}"
        )
        self.stdout.write(
            f"   - Pendientes: {Ticket.objects.filter(estado=Ticket.ESTADO_PENDIENTE).count()}"
        )
        self.stdout.write(
            f"   - Cancelados: {Ticket.objects.filter(estado=Ticket.ESTADO_CANCELADO).count()}"
        )
