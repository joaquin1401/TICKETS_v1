...
from reservas.models import NotificationLog, Ticket
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        q_3 = Ticket.objects.filter(
            estado=Ticket.ESTADO_APROBADO, hora_inicio__date=in_3_days
        )
        count3 = 0
        for t in q_3:
            if not NotificationLog.objects.filter(
                ticket=t, notification_type=NotificationLog.TYPE_REMINDER_3_DAYS
            ).exists():
        ...
        q_today = Ticket.objects.filter(
            estado=Ticket.ESTADO_APROBADO, hora_inicio__date=today
        )
        count_today = 0
        for t in q_today:
            if not NotificationLog.objects.filter(
                ticket=t, notification_type=NotificationLog.TYPE_REMINDER_SAME_DAY
            ).exists():
        ...
        q_late = Ticket.objects.filter(
            estado=Ticket.ESTADO_EN_CURSO, hora_fin__lt=one_hour_ago
        )
        count_late = 0
        for t in q_late:
            if not NotificationLog.objects.filter(
                ticket=t, notification_type=NotificationLog.TYPE_REMINDER_RETURN_LATE
            ).exists():
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Reminders sent: 3-days={count3}, today={count_today}, late={count_late}"
            )
        )
