...
class TestReglasNegocioTickets(TestCase):
    def setUp(self):
        ...
        self.secretario = Usuario.objects.create(
            ...
            id_cargo=self.cargo_secretario,
            valido=True,
        )
    ...
    def test_decano_prioridad_sobre_vehiculo_decanato(self):
        ...
        res_sec = crear_ticket_con_reglas(
            self.secretario,
            self.vehiculo_decanato,
            inicio,
            fin,
            destino="X",
            cant_pasajeros=1,
        ...
        res_decano = crear_ticket_con_reglas(
            self.decano,
            self.vehiculo_decanato,
            inicio,
            fin,
    ...
    def test_decano_sobrescribe_reserva_normal(self):
        ...
        self.assertTrue(
            Ticket.objects.filter(
                id_usuario=self.usuario_comun, estado=Ticket.ESTADO_APROBADO
            ).exists()
        )
        ...
        self.assertTrue(
            Ticket.objects.filter(
                id_usuario=self.usuario_comun, estado=Ticket.ESTADO_CANCELADO
            ).exists()
        )
        self.assertTrue(
            Ticket.objects.filter(
                id_usuario=self.decano, estado=Ticket.ESTADO_APROBADO
            ).exists()
        )
    ...
    def test_feriado_bloquea_incluso_sin_pasar_por_el_form(self):
        ...
        Feriado.objects.create(
            fecha=to_local_date(inicio), descripcion="Feriado de prueba"
        )
    ...
    def test_feriado_no_bloquea_fuera_del_feriado(self):
        ...
        Feriado.objects.create(
            fecha=to_local_date(inicio) + timedelta(days=5), descripcion="Otro día"
        )
    ...
    def test_limite_maximo_anticipacion(self):
        ...
        self.assertEqual(res.estado, ResultadoCreacion.BLOQUEADO)
        self.assertIn(
            "No se pueden realizar reservas con más de 45 días de antelación.",
            res.mensaje,
        )

        inicio_valido = self.ahora + timedelta(days=44)
        res_valido = crear_ticket_con_reglas(
            self.usuario_comun,
            self.vehiculo_normal,
            inicio_valido,
            inicio_valido + timedelta(hours=2),
            destino="X",
            cant_pasajeros=1,
            confirmado=True,
        )
    ...
    def test_anticipacion_minima_cancelacion(self):
        ...
        res_lejos = crear_ticket_con_reglas(
            self.usuario_comun,
            self.vehiculo_normal,
            inicio_lejos,
            inicio_lejos + timedelta(hours=2),
            destino="X",
            cant_pasajeros=1,
            confirmado=True,
        )
        ...
        t_cerca = Ticket.objects.create(
            ...
            id_vehiculo=self.vehiculo_normal,
...
class TestMargenEntreReservas(TestCase):
    ...
    def test_margen_default_1_hora_bloquea_reserva_cercana(self):
        ...
        Ticket.objects.create(
            id_usuario=self.usuario,
            id_vehiculo=self.vehiculo,
            hora_inicio=inicio_existente,
            hora_fin=fin_existente,
            estado=Ticket.ESTADO_APROBADO,
            destino="X",
            cant_pasajeros=1,
        )
    ...
    def test_admin_ignora_margen(self):
        ...
        inicio_nuevo = fin_existente + timedelta(
            minutes=10
        )  # Solo 10 min de diferencia
        fin_nuevo = inicio_nuevo + timedelta(hours=2)
        ...
        res = crear_ticket_con_reglas(
            admin, self.vehiculo, inicio_nuevo, fin_nuevo, destino="Y", cant_pasajeros=1
        )
