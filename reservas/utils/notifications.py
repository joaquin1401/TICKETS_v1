...
def notify_reservation_created(ticket):
    ...
    if NotificationLog.objects.filter(
        ticket=ticket, notification_type=NotificationLog.TYPE_CREATED
    ).exists():
        return
    ...
    async_task(
        ...
        subject,
        "reservas/emails/reservation_created",
        ctx,
        ticket.id_usuario.correo,
    )
...
def notify_reservation_cancelled(ticket):
    ...
    if NotificationLog.objects.filter(
        ticket=ticket, notification_type=NotificationLog.TYPE_CANCELLED
    ).exists():
        return
    ...
    async_task(
        "reservas.tasks.enviar_correo_templated_async",
        subject,
        "reservas/emails/reservation_cancelled",
        ctx,
        ticket.id_usuario.correo,
    )
    NotificationLog.objects.create(
...
def notify_vehicle_inactive_cancelled(
    ...
    async_task(
        "reservas.tasks.enviar_correo_templated_async",
        subject,
        "reservas/emails/vehicle_inactive_cancelled",
        ctx,
        ticket.id_usuario.correo,
    )
    NotificationLog.objects.create(
...
def notify_vehicle_inactive_reassigned(ticket_original, nuevo_ticket, permanente=False):
    ...
    async_task(
        "reservas.tasks.enviar_correo_templated_async",
        subject,
        "reservas/emails/vehicle_inactive_reassigned",
        ctx,
        ticket_original.id_usuario.correo,
    )
    NotificationLog.objects.create(
        ticket=ticket_original, notification_type=NotificationLog.TYPE_REASSIGNED
    )
...
def notify_priority_cancelled(ticket, tiene_permiso=False, dias_gracia=None):
    ...
    """
    if NotificationLog.objects.filter(
        ticket=ticket, notification_type=NotificationLog.TYPE_PRIORITY_CANCELLED
    ).exists():
        return
    ...
    async_task(
        "reservas.tasks.enviar_correo_templated_async",
        subject,
        "reservas/emails/priority_cancelled",
        ctx,
        ticket.id_usuario.correo,
    )
    NotificationLog.objects.create(
...
def notify_priority_reassigned(ticket_original, nuevo_ticket):
    ...
    async_task(
        "reservas.tasks.enviar_correo_templated_async",
        subject,
        "reservas/emails/priority_reassigned",
        ctx,
        ticket_original.id_usuario.correo,
    )
    NotificationLog.objects.create(
